...
def download_csv():
    ...
    for rank, (names, emails, similarity, _) in enumerate(results, start=1):
...
@app.route('/open_pdf/<int:rank>')
def open_pdf(rank):
    # Check if the rank is valid
    if 0 < rank <= len(results):
        # Get the filename of the ranked resume
        filename = results[rank - 1][-1]
